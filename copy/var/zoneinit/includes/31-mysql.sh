...
MYSQL_INIT_DCM4CHEE="create database pacsdb;
GRANT ALL on pacsdb.* to 'pacs'@'127.0.0.1' identified by 'pacs';
FLUSH PRIVILEGES;
use pacsdb;
source /export/dcm4chee/sql/create.mysql;"

log "running dcm4chee database population"
mysql -uroot -p${MYSQL_PW} -e "${MYSQL_INIT_DCM4CHEE}" >/dev/null || \
  ( log "ERROR MySQL dcm4chee stuff failed to execute." && exit 31 )
